...
def register_frontend_extension(config: dict) -> None:
    ...
    if not config.get("id"):
        raise ValueError('[EXT-UI] register_frontend_extension: "id" is required')
    _frontend_extensions.append(config)
    has_tab = bool(config.get("tab_icon") or config.get("tab_label"))
    print(
        f'[EXT-UI] ✅ Frontend extension registered: {config["id"]}'
...
def update_file(file_id):
    ...
    if "folder_id" in data:
        ...
        conn.execute(
            "UPDATE files SET folder_id = ? WHERE id = ?", (new_folder_id, file_id)
        )
...
def batch_update_files():
    ...
    for tag_name in [
        t.strip().lstrip("#") for t in data.get("add_tags", []) if t.strip()
    ]:
        conn.execute("INSERT OR IGNORE INTO tags (name) VALUES (?)", (tag_name,))
        tag_row = conn.execute(
            "SELECT id FROM tags WHERE name = ?", (tag_name,)
    ...
    for tag_name in [
        t.strip().lstrip("#") for t in data.get("remove_tags", []) if t.strip()
    ]:
        tag_row = conn.execute(
            "SELECT id FROM tags WHERE name = ?", (tag_name,)
        ).fetchone()
...
def download_file(file_id):
    ...
    directory = (
        folder_disk_path(row["folder_name"]) if row["folder_name"] else UPLOAD_FOLDER
    )
                ...
                for seek in ("1", "0"):
                    try:
                        ...
                        if proc.returncode == 0 and proc.stdout:
                            resp = send_file(
                                io.BytesIO(proc.stdout), mimetype="image/jpeg"
                            )
...
def _print_startup_banner():
    ...
    try:
        ...
        upload_dir_size = sum(
            ...
            for dp, _, fs in os.walk(UPLOAD_FOLDER)
            for f in fs
        )
    ...
    # System info
    line(
        f'  {DIM}Python {py_ver}   {os_info}   {datetime.now().strftime("%Y-%m-%d %H:%M")}{RESET}'
    )
    ...
    status_row(
        ...
        detail=(
            ", ".join(e["id"] for e in _frontend_extensions) if n_ui_exts > 0 else ""
        ),
